...
def minimal_claude_md(
    vault_slug='test-vault', roles=('alpha', 'beta'), extension_checks_block='extension_checks: []'
):
    role_rows = '\n'.join(f'| `{r}` | def {r} | when {r} |' for r in roles)
...
def build_vault(
    tmp_path, claude_md=None, topic_pages=None, entity_pages=None, index_md=None, log_md=''
):
