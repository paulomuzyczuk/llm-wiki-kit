...
def test_check_4_entity_neighbor_excluded(tmp_path):
    ...
    extra = {
        'p00': ['concept-x', 'bond-markets-fabozzi-book'],
...
def test_check_6_clean():
    ...
    pages, _ = make_pages(
        {
            'wiki/topics/a.md': frontmatter(title='A', status='active', last_updated=fresh),
        }
    )
...
def test_check_6_stale_active():
    pages, _ = make_pages(
        {
            'wiki/topics/a.md': frontmatter(title='A', status='active', last_updated='2000-01-01'),
        }
    )
    res = lint.check_6_stale_claims(pages)
...
def _zero_activity():
    return {'books': 0, 'articles': 0, 'other_ingests': 0, 'handoffs': 0, 'handoff_types': {}}


def test_phase_2_5_consistent_no_fatal():
    # Zero delta -> no FATAL even with watermark + zero activity.
    p2 = {'skipped': False, 'delta': {'alpha': 0, 'beta': 0}, 'reported': {}, 'actual': {}}
...
def test_phase_2_5_fatal_fires():
    ...
    p2 = {'skipped': False, 'delta': {'alpha': 3}, 'reported': {'alpha': 0}, 'actual': {'alpha': 3}}
    fatal = lint.phase_2_5_guard(_zero_activity(), p2, datetime.date(2024, 1, 1))
    assert fatal
...
def test_ext_merge_tombstone_hygiene_violations(tmp_path):
    vault = build_vault(
        ...
        topic_pages={
            ...
            'live': (
                frontmatter(title='Live', status='active', aliases=[]) + '# Live\nsee [[tomb]]\n'
            ),
        },
    )
    findings = lint._ext_merge_tombstone_hygiene(str(vault), {}, {})
...
def test_check_8_variant_use_preferred(tmp_path):
    ...
    pages, _ = make_pages(
        {
            'wiki/topics/a.md': frontmatter(
                title='A', aliases=[], topics=['information-retrieval', 'stub']
            ),
        }
    )
...
def test_check_8_alias_collision_and_shadow(tmp_path):
    ...
    pages, _ = make_pages(
        {
            'wiki/topics/a.md': frontmatter(
                title='Page A', aliases=['foo bar'], topics=['retrieval']
            ),
            'wiki/topics/b.md': frontmatter(
                title='Page B', aliases=['foo bar'], topics=['retrieval']
            ),
    ...
    pages, _ = make_pages(
        {
            ...
            'wiki/topics/b.md': frontmatter(
                title='B', aliases=['catalog rec'], topics=['information-organization', 'stub']
            ),
        }
    )
    r = lint.check_8_vocabulary(pages, auth)
    assert (
        r['topic_unregistered'] == []
        and r['topic_use_preferred'] == []
        and r['alias_collision'] == []
        and r['alias_shadows'] == []
    )
