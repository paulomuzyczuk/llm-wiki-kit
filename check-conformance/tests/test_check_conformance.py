...
import re
import pytest
from conftest import make_vault, run_check, TEST_SUBS, SCRIPT_PATH
...
def test_fully_conformant_exits_0(tmp_path):
    ...
    assert not any(
        ln.startswith("REGION") and "DIFF" in ln
...
def test_one_edited_sentence_exits_1(tmp_path):
    ...
    diff_lines = [ln for ln in result.stdout.splitlines() if ln.startswith("REGION") and "DIFF" in ln]
...
def test_missing_h2_does_not_disturb_later_regions(tmp_path):
    ...
    one_record_lines = [
        ln for ln in result.stdout.splitlines()
        if "One-record" in ln and "REGION" in ln
    ]
...
def test_two_blank_lines_before_sentinel_still_exempt(tmp_path):
    ...
    """
    vault = make_vault()
    # Inject a second blank line immediately before "The `source_tier:` field…"
    # in both the vault AND we need to do the same to the expected file that
    # the script builds from the template.  Since we can only mutate the vault,
    # we mutate the template copy the script uses by also injecting the extra
    # blank into the real CLAUDE.template.md — but that would alter the repo.
    #
    # Cleaner approach: test via the internal API (_find_roles_region directly).
    import importlib.util
    spec = importlib.util.spec_from_file_location("check_conformance", str(SCRIPT_PATH))
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
